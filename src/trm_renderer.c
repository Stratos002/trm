...
void TRM_Renderer_start(GLFWwindow* pWindow, uint32_t windowWidth, uint32_t windowHeight)
{
    ...
    TRM_Memory_allocate(sizeof(VkImage) * pState->swapchainImageCount, (void**)&pState->pSwapchainImages);
    vkGetSwapchainImagesKHR(pState->device, pState->swapchain, &pState->swapchainImageCount, pState->pSwapchainImages);
    TRM_Memory_allocate(sizeof(VkImageView) * pState->swapchainImageCount, (void**)&pState->pSwapchainImageViews);
    ...
    for (uint32_t swapchainImageIndex = 0; swapchainImageIndex < pState->swapchainImageCount; ++swapchainImageIndex)
    {
        TRM_Renderer_createImageView(
            pState->pAllocator, 
            pState->device, 
            pState->pSwapchainImages[swapchainImageIndex], 
            swapchainFormat, 
            &pState->pSwapchainImageViews[swapchainImageIndex]);
    }

    TRM_Renderer_createCommandPool(pState->pAllocator, pState->device, pState->queueFamilyIndex, &pState->commandPool);
    TRM_Renderer_allocateCommandBuffer(pState->commandPool, pState->device, &pState->commandBuffer);
    TRM_Renderer_createFence(pState->pAllocator, pState->device, &pState->commandBufferExecutedFence);
    TRM_Renderer_createSemaphore(pState->pAllocator, pState->device, &pState->imageAvailableSemaphore);
    TRM_Renderer_createSemaphore(pState->pAllocator, pState->device, &pState->imageRenderedSemaphore);
}
...
void TRM_Renderer_terminate()
{
    if (pState != NULL)
    {
        ...
        vkDestroySemaphore(pState->device, pState->imageRenderedSemaphore, pState->pAllocator);
        vkDestroySemaphore(pState->device, pState->imageAvailableSemaphore, pState->pAllocator);
        vkDestroyFence(pState->device, pState->commandBufferExecutedFence, pState->pAllocator);
        vkDestroyCommandPool(pState->device, pState->commandPool, pState->pAllocator);

        for (uint32_t swapchainImageIndex = 0; swapchainImageIndex < pState->swapchainImageCount; ++swapchainImageIndex)
        {
            vkDestroyImageView(pState->device, pState->pSwapchainImageViews[swapchainImageIndex], pState->pAllocator);
        }

        TRM_Memory_deallocate(pState->pSwapchainImageViews);
...
{
    if (vkWaitForFences(pState->device, 1, &pState->commandBufferExecutedFence, VK_FALSE, UINT64_MAX) != VK_SUCCESS)
    {
        exit(EXIT_FAILURE);
    }

    if (vkResetFences(pState->device, 1, &pState->commandBufferExecutedFence) != VK_SUCCESS)
    {
        exit(EXIT_FAILURE);
    }

    uint32_t imageIndex = 0;
    if (vkAcquireNextImageKHR(pState->device, pState->swapchain, UINT64_MAX, pState->imageAvailableSemaphore, VK_NULL_HANDLE, &imageIndex) != VK_SUCCESS)
    {
        exit(EXIT_FAILURE);
    }

    vkResetCommandBuffer(pState->commandBuffer, 0);

    static VkBool32 firstSubmit = VK_TRUE; // par image
    ...
    if (vkBeginCommandBuffer(pState->commandBuffer, &commandBufferBeginInfo) != VK_SUCCESS)
    {
        exit(EXIT_FAILURE);
    }
    ...
    // src present -> transfer opt
    {
        ...
        imageMemoryBarrier.oldLayout = firstSubmit ? VK_IMAGE_LAYOUT_UNDEFINED : VK_IMAGE_LAYOUT_PRESENT_SRC_KHR;
        imageMemoryBarrier.newLayout = VK_IMAGE_LAYOUT_GENERAL;
        imageMemoryBarrier.srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
        imageMemoryBarrier.dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
        imageMemoryBarrier.image = pState->pSwapchainImages[imageIndex];
        ...
        vkCmdPipelineBarrier(pState->commandBuffer, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, NULL, 0, NULL, 1, &imageMemoryBarrier);
    }
    ...
    vkCmdClearColorImage(pState->commandBuffer, pState->pSwapchainImages[imageIndex], VK_IMAGE_LAYOUT_GENERAL, &clearColorValue, 1, &subresourceRange);
    ...
    // transfer dst -> present src
    {
        ...
        vkCmdPipelineBarrier(pState->commandBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, 0, 0, NULL, 0, NULL, 1, &imageMemoryBarrier);
    }
    ...
    submitInfo.pWaitSemaphores = &pState->imageAvailableSemaphore;
    submitInfo.pWaitDstStageMask = &waitDstStageMask;
    submitInfo.pCommandBuffers = &pState->commandBuffer;
    submitInfo.signalSemaphoreCount = 1;
    submitInfo.pSignalSemaphores = &pState->imageRenderedSemaphore;

    if (vkQueueSubmit(pState->queue, 1, &submitInfo, pState->commandBufferExecutedFence) != VK_SUCCESS)
    {
        exit(EXIT_FAILURE);
    }

    vkQueueWaitIdle(pState->queue);

    firstSubmit = VK_FALSE;

    VkPresentInfoKHR presentInfo = {0};
    presentInfo.sType = VK_STRUCTURE_TYPE_PRESENT_INFO_KHR;
    presentInfo.pNext = NULL;
    presentInfo.waitSemaphoreCount = 1;
    presentInfo.pWaitSemaphores = &pState->imageRenderedSemaphore;
